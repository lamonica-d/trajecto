...
#######################################################################################
#######################################################################################
# Script 2 - Data preparation - generating one file per individual usable for JAGS inference
#######################################################################################
#######################################################################################
...
## loading unique .csv data file
data8_sup2=read.table(file="data/derived-data/data_tous_individus_ready_to_use.csv", header=T, sep=",")[,-1]

## vector of the number of trajectories per individual, to save
...
save(nbsection, file = paste0("data/derived-data/nb_section"))
...
## organise as two matrices, to save
...
save(mat_debut, file = paste0("data/derived-data/mat_debut"))
save(mat_fin, file = paste0("data/derived-data/mat_fin"))

## generating and saving one file per individual of interest
numpoiss_vect_int=c(3128,3100,3240,3183,3170,3121,3744,3632,3730,3212,3856,3352,
                    3849,3835,3758,3786,3464,3870)

for (k in 1:length(numpoiss_vect_int)){
  
  id_poiss_int=numpoiss_vect_int[k]
  data_ind=data8_sup2[data8_sup2$id_poiss==id_poiss_int,]
  ...
  save(data_ind, file = paste0("data/derived-data/data_ind",id_poiss_int,sep=""))
