
#######################################################################################
#######################################################################################
#titre
#######################################################################################
#######################################################################################

rm(list=ls())


### loading and preparing data
numpoiss_vect=c(3100,3156,3744,3128,3170,3051,3317,3674,3828,3856,3632,3786,3835,3849,3870,3772,3800,3387,3415,3429,
                3183,3212,3240,3352,3464,3730,3758,3121,3394,3408,3422,3506,3562,3590,3625,3079,3303)

data8_sup2=read.table(file="data_tous_individus_ready_to_use_v4.csv", header=T, sep=",")[,-1]

###choose which individual
num_ind=1
poiss=1:length(numpoiss_vect)

#on refait le coup des section

fin_poiss_sup2=sapply(poiss,function(s) max(which(data8_sup2$numpoiss==s)))

nbsection=numeric(length(poiss))
for (i in 1:length(poiss)){
  temp1=data8_sup2[data8_sup2$numpoiss==i,]
  nbsection[i]=length(unique(temp1$sqcont_new))
}

section=list()
debut_section=list()
fin_section=list()

temp1=data8_sup2[data8_sup2$numpoiss==1,]
section[[1]]=unique(temp1$sqcont_new)
debut_section[[1]]=sapply(section[[1]],function(s) min(which(temp1$sqcont_new==s)))
fin_section[[1]]=sapply(section[[1]],function(s) max(which(temp1$sqcont_new==s)))


for (i in 2:length(poiss)){
  temp1=data8_sup2[data8_sup2$numpoiss==i,]
  section[[i]]=unique(temp1$sqcont_new)
  debut_section[[i]]=sapply(section[[i]],function(s) min(which(temp1$sqcont_new==s)))+fin_poiss_sup2[i-1]
  fin_section[[i]]=sapply(section[[i]],function(s) max(which(temp1$sqcont_new==s)))+fin_poiss_sup2[i-1]
}


mat_debut=matrix(data=0, nrow=length(poiss), ncol=max(nbsection))
mat_fin=matrix(data=0, nrow=length(poiss), ncol=max(nbsection))
for (i in 1:length(poiss)){
  
  mat_debut[i,]=c(unlist(debut_section[[i]]), rep(0,max(nbsection)-length(unlist(debut_section[[i]]))))
  mat_fin[i,]=c(unlist(fin_section[[i]]), rep(0,max(nbsection)-length(unlist(fin_section[[i]]))))
}

#selectionner le df de l'individu qu'on traite
data_ind=data8_sup2[data8_sup2$numpoiss==num_ind,]
rm(data8_sup2)

##################pb NA qui trainent#######################
#sil
if (numpoiss_vect[num_ind]==3786){data_ind$temp[1315]=data_ind$temp[1314]
data_ind$temp_carre[1315]=data_ind$temp_carre[1314]}
if (numpoiss_vect[num_ind]==3835){data_ind$temp[11712]=data_ind$temp[11711]
data_ind$temp_carre[11712]=data_ind$temp_carre[11711]}
if (numpoiss_vect[num_ind]==3849){data_ind$temp[4043]=data_ind$temp[4042]
data_ind$temp_carre[4043]=data_ind$temp_carre[4042]}
if (numpoiss_vect[num_ind]==3387){data_ind$temp[c(73,74,182:185,207,211:214)]=
  data_ind$temp[c(72,72,rep(181,4),206,rep(210,4))]
data_ind$temp_carre[c(73,74,182:185,207,211:214)]=
  data_ind$temp_carre[c(72,72,rep(181,4),206,rep(210,4))]}

#baf
if (numpoiss_vect[num_ind]==3128){data_ind$temp[7363]=data_ind$temp[7362]
data_ind$hmod[7363]=data_ind$hmod[7362]
data_ind$vmod[7363]=data_ind$vmod[7362]

data_ind$temp_carre[7363]=data_ind$temp_carre[7362]
data_ind$hmod_carre[7363]=data_ind$hmod_carre[7362]
data_ind$vmod_carre[7363]=data_ind$vmod_carre[7362]}

if (numpoiss_vect[num_ind]==3170){data_ind$temp[c(1034,1236,1706,1707,9957,9967)]=data_ind$temp[c(1034,1236,1706,1706,9957,9967)-1]
data_ind$hmod[c(9957,9967)]=data_ind$hmod[c(9957,9967)-1]
data_ind$vmod[c(9957,9967)]=data_ind$vmod[c(9957,9967)-1]

data_ind$temp_carre[c(1034,1236,1706,1707,9957,9967)]=data_ind$temp_carre[c(1034,1236,1706,1706,9957,9967)-1]
data_ind$hmod_carre[c(9957,9967)]=data_ind$hmod_carre[c(9957,9967)-1]
data_ind$vmod_carre[c(9957,9967)]=data_ind$vmod_carre[c(9957,9967)-1]}

if (numpoiss_vect[num_ind]==3744){data_ind$temp[c(93,96,99,103,104,129,132,142,146,150,151,174,175,184,190:192,195,198,199,218,221,230,233,236,239,245,263,264,272:275,1316,1323,1324,1344,1345,
                                                  1349,1406,1442,1454,1484,1770,1774,1791:1793,1808,1809,1826,1827,1848,1849,1860,1870:1872,1895,1907,1913,1919,1931:1933,1937:1940,1943,
                                                  1951,1964,1965,1968,1969,1972,1973,4310:4314,4317,4318,4321,4335,4346:4349,4353,4360,4393,4397,4398,4402,4440,4448,4449,4457:4459,4465,4466,
                                                  4477,4501,4502,4525,4530,4536,4543:4549,4554:4557,4562,4565,4568,4569,4576:4579,4593:4596,4603:4606,4609,4615,4616,4677,4678,4681:4684,4731,
                                                  4765,4768,4793,4811,4826,4843,4848,4853,4868,4871,4892,4898,4905,4912,4913,4929:4931,4952,4966,4973,4974,4978,4992:4995,5002:5006,5162,5171:5174,
                                                  5186:5189,5194,5199,5206,5209,5223,5227,5228,5242,5246:5248,5253,5254,5258,5696,5697,5741,5862,5880,6379,6454,6455,6678:6681,6690,6693:6698,6710,
                                                  6714,6739,6740,6886,6934,6938:6941,7177,7369,7420,7493,7501,7502,7521,7541,7563,7599,7614,7717:7719,7729,7737,7773,7869,7983,8006,8015,8022,8090,
                                                  8155,8185,8200,8317,8322,8366,8391,8394,8395,8399,8494,8508,8512,8929,8930,8985,8998,9689,9729,12694,12695,12698:12700,12923,13053,13318,13382,
                                                  13722,16596,17489,17576:17578,17601,17608,20877,20891,20894,21057,21261,21554,23169,23696,24305,24391,24395,24398,24399,24402,24420,24421,24568,
                                                  24805,25593,27167,27801,28477,28495:28498,33738,34177,34247,34365,43275,43524,43571,43572,43576:43578)]=
  data_ind$temp[c(c(93,96,99,103)-1,102,c(129,132,142,146,150)-1,149,173,173,183,rep(189,3),194,197,197,c(218,221,230,233,236,239,245,263)-1,262,rep(271,4),1315,1322,1322,1343,1343,
                  c(1349,1406,1442,1454,1484,1770,1774)-1,rep(1790,3),1807,1807,1825,1825,1847,1847,1859,rep(1869,3),c(1895,1907,1913,1919)-1,rep(1930,3),rep(1936,4),c(1943,
                                                                                                                                                                        1951,1964)-1,1963,1967,1967,1971,1971,rep(4309,5),4316,4316,4320,4334,rep(4345,4),c(4353,4360,4393,4397)-1,4396,c(4402,4440,4448)-1,4447,
                  rep(4456,3),4464,4464,4476,4500,4500,c(4525,4530,4536)-1,rep(4542,7),rep(4553,4),4561,4564,4567,4567,rep(4575,4),rep(4592,4),rep(4602,4),4608,4614,4614,4676,4676,rep(4680,4)
                  ,c(4731,4765,4768,4793,4811,4826,4843,4848,4853,4868,4871,4892,4898,4905,4912)-1,4911,rep(4928,3),4951,4965,4972,4972,4977,rep(4991,4),rep(5001,5),5161,rep(5170,4),
                  rep(5185,4),c(5194,5199,5206,5209,5223,5227)-1,5226,5241,rep(5245,3),5252,5252,5257,5695,5695,c(5741,5862,5880,6379,6454)-1,6453,rep(6677,4),6689,rep(6692,6),
                  c(6710,6714,6739)-1,6738,6885,6933,rep(6937,4),c(7177,7369,7420,7493,7501)-1,7500,c(7521,7541,7563,7599,7614)-1,rep(7716,3),c(7729,7737,7773,7869,7983,8006,8015,8022,8090,
                                                                                                                                                8155,8185,8200,8317,8322,8366,8391,8394)-1,8393,c(8399,8494,8508,8512,8929)-1,8928,c(8985,8998,9689,9729,12694)-1,12693,rep(12697,3),c(12923,13053,13318,13382,
                                                                                                                                                                                                                                                                                       13722,16596,17489)-1,rep(17575,3),c(17601,17608,20877,20891,20894,21057,21261,21554,23169,23696,24305,24391,24395,24398)-1,24397,24401,24419,24419,c(24568,
                                                                                                                                                                                                                                                                                                                                                                                                                                            24805,25593,27167,27801,28477)-1,rep(28494,4),c(33738,34177,34247,34365,43275,43524,43571)-1,43570,rep(43575,3))]



data_ind$hmod[c(299,1968,4353,4393,4993,5157,5162,5163,5171:5174,5186:5189,5194,5198,5199,5204:5206,5209:5213,5223,5224,5227,5228,5241,5242,5246:5248,5253:5255,5258,
                5862,5879,5880,6454,6455,6678:6681,6685,6690:6700,6709,6710,6714,6739,6740,6835,6858,6886,6934,6938:6941,6948,7082,7088,7177,7369,7718,7719,7729,7737,
                7749:7751,7754,7773,7869,8022,8366,8494,8508,8512,8924,8929:8933,8985,8998:9000,9232,9729,9973,9974,9979,10121,10330,10709,10742,10743,11991,12052,12056,
                12061,12064,12065,12843:12846,12854,12859,12919:12923,12929,12930,12982,12985,12990,13053,13059,13721,13722,13773,13941,14041,14042,14244,14245,14252,14775,
                14778,15460,15461,16593:16596,16609:16618,17576,17577,17671:17673,17678,17679,17702,17731,17748,17778,17785,17804,17808,17815,17820,17852:17855,17858,17861:17864,
                17898,17899,17920,17924,17942,17952:17955,17961:17963,17969,17970,17979,17982,18994:18996,19025,19026,19039,19051:19053,20869,20877,20891,20894,21057,21497,
                21554,21922,23169,24381,24391,24394,24395,24399,24402,24405,24412,24420,24421,24961,27801,28495,29770,32066,43523,43524)]=
  
  
  data_ind$hmod[c(c(299,1968,4353,4393,4993,5157,5162)-1,5161,rep(5170,4),rep(5185,4),5193,5197,5197,rep(5203,3),rep(5208,5),5222,5222,5226,5226,5240,5240,rep(5245,3),rep(5252,3),5257,
                  5861,5878,5878,6453,6453,rep(6677,4),6684,rep(6689,11),6708,6708,6713,6738,6738,c(6835,6858,6886,6934)-1,rep(6937,4),c(6948,7082,7088,7177,7369,7718)-1,
                  7717,7728,7736,rep(7748,3),c(7754,7773,7869,8022,8366,8494,8508,8512,8924)-1,rep(8928,5),8984,rep(8997,3),c(9232,9729,9973)-1,9972,c(9979,10121,10330,10709,
                                                                                                                                                       10742)-1,10741,c(11991,12052,12056,12061,12064)-1,12063,rep(12842,4),12853,12858,rep(12918,5),12928,12928,c(12982,12985,12990,13053,13059,13721)-1,13720,
                  c(13773,13941,14041)-1,14040,14243,14243,c(14252,14775,14778,15460)-1,15459,rep(16592,4),rep(16608,10),17575,17575,rep(17670,3),17677,17677,c(17702,17731,17748,
                                                                                                                                                                17778,17785,17804,17808,17815,17820)-1,rep(17851,4),17857,rep(17860,4),17897,17897,c(17920,17924,17942)-1,rep(17951,4),rep(17960,3),17968,17968,c(17979,17982)-1,
                  rep(18993,3),19024,19024,19038,rep(19050,3),c(20869,20877,20891,20894,21057,21497,21554,21922,23169,24381,24391,24394)-1,24393,c(24399,24402,24405,24412,24420)-1,24419,c(24961,27801,28495,29770,32066,43523)-1,43522)]


data_ind$vmod[c(4353,4393,4993,5173,5174,5206,5209,5223,5227,5228,5247,5248,5253,5254,5862,5879,5880,6454,6455,6678:6681,6690,6694:6698,6710,6714,6739,6740,6858,6886,6934,
                6939:6941,7082,7088,7177,7369,7737,7749,7750,8022,8366,8494,8929,8930,8998,9729,9974,10742,10743,12064,12065,12844:12846,12859,12919:12924,12982,12990,13053,
                13059,13722,13773,14041,14042,14244,14245,14778,16593:16596,16609,16612:16617,17576,17577,17671,17678,17679,17731,17852:17854,17858,17862,17863,17924,17982,
                18994:18996,19025,19026,19039,20869,20891,21057,21554,24391,24395,24402,27801,29770)]=
  
  data_ind$vmod[c(
    c(4353,4393,4993,5173)-1,5172,c(5206,5209,5223,5227)-1,5226,5246,5246,5252,5252,5861,5878,5878,6453,6453,rep(6677,4),6689,rep(6693,5),c(6710,6714,6739)-1,6738,
    c(6858,6886,6934)-1,rep(6938,3),c(7082,7088,7177,7369,7737,7749)-1,7748,c(8022,8366,8494,8929)-1,8928,c(8998,9729,9974,10742)-1,10741,12063,12063,rep(12843,3),12858,
    rep(12918,6),c(12982,12990,13053,13059,13722)-1,13771,14040,14040,14243,14243,14777,rep(16592,4),16608,rep(16611,6),17575,17575,17670,17677,17677,17730,rep(17851,3),
    17857,17861,17861,17923,17981,rep(18993,3),19024,19024,c(19039,20869,20891,21057,21554,24391,24395,24402,27801,29770)-1)]

data_ind$temp_carre[c(93,96,99,103,104,129,132,142,146,150,151,174,175,184,190:192,195,198,199,218,221,230,233,236,239,245,263,264,272:275,1316,1323,1324,1344,1345,
                      1349,1406,1442,1454,1484,1770,1774,1791:1793,1808,1809,1826,1827,1848,1849,1860,1870:1872,1895,1907,1913,1919,1931:1933,1937:1940,1943,
                      1951,1964,1965,1968,1969,1972,1973,4310:4314,4317,4318,4321,4335,4346:4349,4353,4360,4393,4397,4398,4402,4440,4448,4449,4457:4459,4465,4466,
                      4477,4501,4502,4525,4530,4536,4543:4549,4554:4557,4562,4565,4568,4569,4576:4579,4593:4596,4603:4606,4609,4615,4616,4677,4678,4681:4684,4731,
                      4765,4768,4793,4811,4826,4843,4848,4853,4868,4871,4892,4898,4905,4912,4913,4929:4931,4952,4966,4973,4974,4978,4992:4995,5002:5006,5162,5171:5174,
                      5186:5189,5194,5199,5206,5209,5223,5227,5228,5242,5246:5248,5253,5254,5258,5696,5697,5741,5862,5880,6379,6454,6455,6678:6681,6690,6693:6698,6710,
                      6714,6739,6740,6886,6934,6938:6941,7177,7369,7420,7493,7501,7502,7521,7541,7563,7599,7614,7717:7719,7729,7737,7773,7869,7983,8006,8015,8022,8090,
                      8155,8185,8200,8317,8322,8366,8391,8394,8395,8399,8494,8508,8512,8929,8930,8985,8998,9689,9729,12694,12695,12698:12700,12923,13053,13318,13382,
                      13722,16596,17489,17576:17578,17601,17608,20877,20891,20894,21057,21261,21554,23169,23696,24305,24391,24395,24398,24399,24402,24420,24421,24568,
                      24805,25593,27167,27801,28477,28495:28498,33738,34177,34247,34365,43275,43524,43571,43572,43576:43578)]=
  data_ind$temp_carre[c(c(93,96,99,103)-1,102,c(129,132,142,146,150)-1,149,173,173,183,rep(189,3),194,197,197,c(218,221,230,233,236,239,245,263)-1,262,rep(271,4),1315,1322,1322,1343,1343,
                        c(1349,1406,1442,1454,1484,1770,1774)-1,rep(1790,3),1807,1807,1825,1825,1847,1847,1859,rep(1869,3),c(1895,1907,1913,1919)-1,rep(1930,3),rep(1936,4),c(1943,
                                                                                                                                                                              1951,1964)-1,1963,1967,1967,1971,1971,rep(4309,5),4316,4316,4320,4334,rep(4345,4),c(4353,4360,4393,4397)-1,4396,c(4402,4440,4448)-1,4447,
                        rep(4456,3),4464,4464,4476,4500,4500,c(4525,4530,4536)-1,rep(4542,7),rep(4553,4),4561,4564,4567,4567,rep(4575,4),rep(4592,4),rep(4602,4),4608,4614,4614,4676,4676,rep(4680,4)
                        ,c(4731,4765,4768,4793,4811,4826,4843,4848,4853,4868,4871,4892,4898,4905,4912)-1,4911,rep(4928,3),4951,4965,4972,4972,4977,rep(4991,4),rep(5001,5),5161,rep(5170,4),
                        rep(5185,4),c(5194,5199,5206,5209,5223,5227)-1,5226,5241,rep(5245,3),5252,5252,5257,5695,5695,c(5741,5862,5880,6379,6454)-1,6453,rep(6677,4),6689,rep(6692,6),
                        c(6710,6714,6739)-1,6738,6885,6933,rep(6937,4),c(7177,7369,7420,7493,7501)-1,7500,c(7521,7541,7563,7599,7614)-1,rep(7716,3),c(7729,7737,7773,7869,7983,8006,8015,8022,8090,
                                                                                                                                                      8155,8185,8200,8317,8322,8366,8391,8394)-1,8393,c(8399,8494,8508,8512,8929)-1,8928,c(8985,8998,9689,9729,12694)-1,12693,rep(12697,3),c(12923,13053,13318,13382,
                                                                                                                                                                                                                                                                                             13722,16596,17489)-1,rep(17575,3),c(17601,17608,20877,20891,20894,21057,21261,21554,23169,23696,24305,24391,24395,24398)-1,24397,24401,24419,24419,c(24568,
                                                                                                                                                                                                                                                                                                                                                                                                                                                  24805,25593,27167,27801,28477)-1,rep(28494,4),c(33738,34177,34247,34365,43275,43524,43571)-1,43570,rep(43575,3))]



data_ind$hmod_carre[c(299,1968,4353,4393,4993,5157,5162,5163,5171:5174,5186:5189,5194,5198,5199,5204:5206,5209:5213,5223,5224,5227,5228,5241,5242,5246:5248,5253:5255,5258,
                      5862,5879,5880,6454,6455,6678:6681,6685,6690:6700,6709,6710,6714,6739,6740,6835,6858,6886,6934,6938:6941,6948,7082,7088,7177,7369,7718,7719,7729,7737,
                      7749:7751,7754,7773,7869,8022,8366,8494,8508,8512,8924,8929:8933,8985,8998:9000,9232,9729,9973,9974,9979,10121,10330,10709,10742,10743,11991,12052,12056,
                      12061,12064,12065,12843:12846,12854,12859,12919:12923,12929,12930,12982,12985,12990,13053,13059,13721,13722,13773,13941,14041,14042,14244,14245,14252,14775,
                      14778,15460,15461,16593:16596,16609:16618,17576,17577,17671:17673,17678,17679,17702,17731,17748,17778,17785,17804,17808,17815,17820,17852:17855,17858,17861:17864,
                      17898,17899,17920,17924,17942,17952:17955,17961:17963,17969,17970,17979,17982,18994:18996,19025,19026,19039,19051:19053,20869,20877,20891,20894,21057,21497,
                      21554,21922,23169,24381,24391,24394,24395,24399,24402,24405,24412,24420,24421,24961,27801,28495,29770,32066,43523,43524)]=
  
  
  data_ind$hmod_carre[c(c(299,1968,4353,4393,4993,5157,5162)-1,5161,rep(5170,4),rep(5185,4),5193,5197,5197,rep(5203,3),rep(5208,5),5222,5222,5226,5226,5240,5240,rep(5245,3),rep(5252,3),5257,
                        5861,5878,5878,6453,6453,rep(6677,4),6684,rep(6689,11),6708,6708,6713,6738,6738,c(6835,6858,6886,6934)-1,rep(6937,4),c(6948,7082,7088,7177,7369,7718)-1,
                        7717,7728,7736,rep(7748,3),c(7754,7773,7869,8022,8366,8494,8508,8512,8924)-1,rep(8928,5),8984,rep(8997,3),c(9232,9729,9973)-1,9972,c(9979,10121,10330,10709,
                                                                                                                                                             10742)-1,10741,c(11991,12052,12056,12061,12064)-1,12063,rep(12842,4),12853,12858,rep(12918,5),12928,12928,c(12982,12985,12990,13053,13059,13721)-1,13720,
                        c(13773,13941,14041)-1,14040,14243,14243,c(14252,14775,14778,15460)-1,15459,rep(16592,4),rep(16608,10),17575,17575,rep(17670,3),17677,17677,c(17702,17731,17748,
                                                                                                                                                                      17778,17785,17804,17808,17815,17820)-1,rep(17851,4),17857,rep(17860,4),17897,17897,c(17920,17924,17942)-1,rep(17951,4),rep(17960,3),17968,17968,c(17979,17982)-1,
                        rep(18993,3),19024,19024,19038,rep(19050,3),c(20869,20877,20891,20894,21057,21497,21554,21922,23169,24381,24391,24394)-1,24393,c(24399,24402,24405,24412,24420)-1,24419,c(24961,27801,28495,29770,32066,43523)-1,43522)]


data_ind$vmod_carre[c(4353,4393,4993,5173,5174,5206,5209,5223,5227,5228,5247,5248,5253,5254,5862,5879,5880,6454,6455,6678:6681,6690,6694:6698,6710,6714,6739,6740,6858,6886,6934,
                      6939:6941,7082,7088,7177,7369,7737,7749,7750,8022,8366,8494,8929,8930,8998,9729,9974,10742,10743,12064,12065,12844:12846,12859,12919:12924,12982,12990,13053,
                      13059,13722,13773,14041,14042,14244,14245,14778,16593:16596,16609,16612:16617,17576,17577,17671,17678,17679,17731,17852:17854,17858,17862,17863,17924,17982,
                      18994:18996,19025,19026,19039,20869,20891,21057,21554,24391,24395,24402,27801,29770)]=
  
  data_ind$vmod_carre[c(
    c(4353,4393,4993,5173)-1,5172,c(5206,5209,5223,5227)-1,5226,5246,5246,5252,5252,5861,5878,5878,6453,6453,rep(6677,4),6689,rep(6693,5),c(6710,6714,6739)-1,6738,
    c(6858,6886,6934)-1,rep(6938,3),c(7082,7088,7177,7369,7737,7749)-1,7748,c(8022,8366,8494,8929)-1,8928,c(8998,9729,9974,10742)-1,10741,12063,12063,rep(12843,3),12858,
    rep(12918,6),c(12982,12990,13053,13059,13722)-1,13771,14040,14040,14243,14243,14777,rep(16592,4),16608,rep(16611,6),17575,17575,17670,17677,17677,17730,rep(17851,3),
    17857,17861,17861,17923,17981,rep(18993,3),19024,19024,c(19039,20869,20891,21057,21554,24391,24395,24402,27801,29770)-1)]}



#che
if (numpoiss_vect[num_ind]==3183){data_ind$temp[c(1516,1517,1518,1522)]=data_ind$temp[c(rep(1515,3), 1521)]
data_ind$hmod[c(1516,1517,1518,1521,1522)]=data_ind$hmod[c(rep(1515,3), 1520,1520)]
data_ind$vmod[c(1516,1517,1518,1522)]=data_ind$vmod[c(rep(1515,3), 1521)]

data_ind$temp_carre[c(1516,1517,1518,1522)]=data_ind$temp_carre[c(rep(1515,3), 1521)]
data_ind$hmod_carre[c(1516,1517,1518,1521,1522)]=data_ind$hmod_carre[c(rep(1515,3), 1520,1520)]
data_ind$vmod_carre[c(1516,1517,1518,1522)]=data_ind$vmod_carre[c(rep(1515,3), 1521)]}

if (numpoiss_vect[num_ind]==3240){data_ind$temp[c(2505,4779,8117,8317)]=data_ind$temp[c(2505,4779,8117,8317)-1]
data_ind$hmod[c(4779,4780)]=data_ind$hmod[c(4778,4778)]
data_ind$vmod[c(4779,4780)]=data_ind$vmod[c(4778,4778)]

data_ind$temp_carre[c(2505,4779,8117,8317)]=data_ind$temp_carre[c(2505,4779,8117,8317)-1]
data_ind$hmod_carre[c(4779,4780)]=data_ind$hmod_carre[c(4778,4778)]
data_ind$vmod_carre[c(4779,4780)]=data_ind$vmod_carre[c(4778,4778)]}

if (numpoiss_vect[num_ind]==3464){data_ind$temp[c(400,414,436,474,489,490)]=data_ind$temp[c(c(400,414,436,474)-1,488,488)]
data_ind$hmod[c(207:209,218,222:229,235,245,252,257,258,297,300:303,324,386,400:402,
                414:416,426,435,436,455,474,475,484,489,490,580:582,603:605)]=
  data_ind$hmod[c(rep(206,3),217,rep(221,8),c(235,245,252)-1,256,256,296,rep(299,4),323,385,rep(399,3),rep(413,3),
                  425,434,434,454,473,473,483,488,488,rep(579,3),rep(602,3))]
data_ind$vmod[c(218,223:227,245,297,303,400:402,414:416,435,436,474,489,490,605)]=
  data_ind$vmod[c(217,rep(222,5),c(245,297,303)-1,rep(399,3),rep(413,3),434,434,473,488,488,604)]

data_ind$temp_carre[c(400,414,436,474,489,490)]=data_ind$temp_carre[c(c(400,414,436,474)-1,488,488)]
data_ind$hmod_carre[c(207:209,218,222:229,235,245,252,257,258,297,300:303,324,386,400:402,
                      414:416,426,435,436,455,474,475,484,489,490,580:582,603:605)]=
  data_ind$hmod_carre[c(rep(206,3),217,rep(221,8),c(235,245,252)-1,256,256,296,rep(299,4),323,385,rep(399,3),rep(413,3),
                        425,434,434,454,473,473,483,488,488,rep(579,3),rep(602,3))]
data_ind$vmod_carre[c(218,223:227,245,297,303,400:402,414:416,435,436,474,489,490,605)]=
  data_ind$vmod_carre[c(217,rep(222,5),c(245,297,303)-1,rep(399,3),rep(413,3),434,434,473,488,488,604)]}

if (numpoiss_vect[num_ind]==3730){data_ind$temp[c(264,563,1239,1568,1598,2837,3239,3361,3743,3744,18544,18548,18549,18556,18568,21822,21845,22151,22167,22171,22520,23128,23146,
                                                  23149:23151,23590,23596:23598,23605,23859,23860)]=data_ind$temp[c(c(264,563,1239,1568,1598,2837,3239,3361,3743)-1,3742,
                                                                                                                    18543,18547,18547,c(18556,18568,21822,21845,22151,22167,22171,22520,23128,23146)-1,rep(23148,3),23589,rep(23595,3),23604,23858,23858)]
data_ind$hmod[c(1608,1609,2837,3239,3361,3743,3744,14522,18556,21845,22151,23859,23860)]=data_ind$hmod[c(1607,1607,2836,3238,3360,3742,3742,c(14522,18556,21845,22151,23859)-1,23858)]
data_ind$vmod[c(1608,1609,2837,3239,3361,14522,18556,21845,23859,23860)]=data_ind$vmod[c(1607,1607,c(2837,3239,3361,14522,18556,21845,23859)-1,23858)]

data_ind$temp_carre[c(264,563,1239,1568,1598,2837,3239,3361,3743,3744,18544,18548,18549,18556,18568,21822,21845,22151,22167,22171,22520,23128,23146,
                      23149:23151,23590,23596:23598,23605,23859,23860)]=data_ind$temp_carre[c(c(264,563,1239,1568,1598,2837,3239,3361,3743)-1,3742,
                                                                                              18543,18547,18547,c(18556,18568,21822,21845,22151,22167,22171,22520,23128,23146)-1,rep(23148,3),23589,rep(23595,3),23604,23858,23858)]
data_ind$hmod_carre[c(1608,1609,2837,3239,3361,3743,3744,14522,18556,21845,22151,23859,23860)]=data_ind$hmod_carre[c(1607,1607,2836,3238,3360,3742,3742,c(14522,18556,21845,22151,23859)-1,23858)]
data_ind$vmod_carre[c(1608,1609,2837,3239,3361,14522,18556,21845,23859,23860)]=data_ind$vmod_carre[c(1607,1607,c(2837,3239,3361,14522,18556,21845,23859)-1,23858)]}

if (numpoiss_vect[num_ind]==3758){data_ind$temp[c(2562,2577,2580,2591,2596,7235,7240,7241,7270)]=data_ind$temp[c(c(2562,2577,2580,2591,2596,7235,7240)-1,7239,7269)]
data_ind$hmod[c(41,2562,2577,2580,2584,2589:2591,2596)]=data_ind$hmod[c(c(41,2562,2577,2580,2584)-1,rep(2588,3),2595)]
data_ind$vmod[c(2562,2577,2580,2591,2596)]=data_ind$vmod[c(2562,2577,2580,2591,2596)-1]

data_ind$temp_carre[c(2562,2577,2580,2591,2596,7235,7240,7241,7270)]=data_ind$temp_carre[c(c(2562,2577,2580,2591,2596,7235,7240)-1,7239,7269)]
data_ind$hmod_carre[c(41,2562,2577,2580,2584,2589:2591,2596)]=data_ind$hmod_carre[c(c(41,2562,2577,2580,2584)-1,rep(2588,3),2595)]
data_ind$vmod_carre[c(2562,2577,2580,2591,2596)]=data_ind$vmod_carre[c(2562,2577,2580,2591,2596)-1]}

if (numpoiss_vect[num_ind]==3121){data_ind$temp[c(1924:1933,4192,4196,4197,4206:4212,4217,4223:4225,4232:4251,4283:4285,4302,4307,4308,4315,4334:4343,
                                                  4358:4362,4365:4368,4371:4375,4378:4391,4394,4399,4400,4439:4442,4445:4447,4566,4571)]=
  data_ind$temp[c(rep(1923,10),4191,4195,4195,rep(4205,7),4216,rep(4222,3),rep(4231,20),rep(4282,3),4301,4306,4306,4314,rep(4333,10),rep(4357,5),
                  rep(4364,4),rep(4370,5),rep(4370,14),4393,4398,4398,rep(4438,4),rep(4444,3),4565,4570)]
data_ind$hmod[c(91:93)]=data_ind$hmod[rep(90,3)]
data_ind$vmod[c(91:93)]=data_ind$vmod[rep(90,3)]

data_ind$temp_carre[c(1924:1933,4192,4196,4197,4206:4212,4217,4223:4225,4232:4251,4283:4285,4302,4307,4308,4315,4334:4343,
                      4358:4362,4365:4368,4371:4375,4378:4391,4394,4399,4400,4439:4442,4445:4447,4566,4571)]=
  data_ind$temp_carre[c(rep(1923,10),4191,4195,4195,rep(4205,7),4216,rep(4222,3),rep(4231,20),rep(4282,3),4301,4306,4306,4314,rep(4333,10),rep(4357,5),
                        rep(4364,4),rep(4370,5),rep(4370,14),4393,4398,4398,rep(4438,4),rep(4444,3),4565,4570)]
data_ind$hmod_carre[c(91:93)]=data_ind$hmod_carre[rep(90,3)]
data_ind$vmod_carre[c(91:93)]=data_ind$vmod_carre[rep(90,3)]}



for (i in which(is.na(data_ind$temp))){
  data_ind$temp[i]<-(data_ind$temp[i-1]+data_ind$temp[i+1])/2
}
for (i in which(is.na(data_ind$hmod))){
  data_ind$hmod[i]<-(data_ind$hmod[i-1]+data_ind$hmod[i+1])/2
}
for (i in which(is.na(data_ind$vmod))){
  data_ind$vmod[i]<-(data_ind$vmod[i-1]+data_ind$vmod[i+1])/2
}

sum(is.na(data_ind$vmod))
sum(is.na(data_ind$hmod))
sum(is.na(data_ind$temp))

# for (i in which(is.na(data_ind$temp_carre))){
#   data_ind$temp_carre[i]<-(data_ind$temp_carre[i-1]+data_ind$temp_carre[i+1])/2
# }
# for (i in which(is.na(data_ind$hmod_carre))){
#   data_ind$hmod_carre[i]<-(data_ind$hmod_carre[i-1]+data_ind$hmod_carre[i+1])/2
# }
# for (i in which(is.na(data_ind$vmod_carre))){
#   data_ind$vmod_carre[i]<-(data_ind$vmod_carre[i-1]+data_ind$vmod_carre[i+1])/2
# }
# 
# sum(is.na(data_ind$vmod_carre))
# sum(is.na(data_ind$hmod_carre))
# sum(is.na(data_ind$temp_carre))


########################nyct#################################
aube=ifelse(data_ind$nyct==1,1,0)
crep=ifelse(data_ind$nyct==2,1,0)
jour=ifelse(data_ind$nyct==3,1,0)
nuit=ifelse(data_ind$nyct==4,1,0)

#############################################################

mydata <- list(qinit=c(0.5,0.5),  nbsection=nbsection[num_ind],pi=pi,#nbpoiss=nbpoiss,
               debut=mat_debut[num_ind,1:nbsection[num_ind]]-mat_debut[num_ind,1]+1,
               fin=mat_fin[num_ind,1:nbsection[num_ind]]-mat_debut[num_ind,1]+1,
               rangle=as.vector(data_ind$rangle),vit=as.vector(data_ind$vitesse)*1000,
               rapport_dist=as.vector(data_ind$rapport_dist), var_angles_bruts=data_ind$var_angles_bruts
               
               ,vitc_moy=as.vector(data_ind$vmod) #, vmod_carre=as.vector(data_ind$vmod)^2
               ,hmod_moy=as.vector(data_ind$hmod)#, hmod_carre=as.vector(data_ind$hmod)^2
               ,temp_moy=as.vector(data_ind$temp) #,temp_carre=as.vector(data_ind$temp)^2
               
               ,aube=as.vector(aube),crep=as.vector(crep),jour=as.vector(jour),nuit=as.vector(nuit)
               
               #,vmax_moy=as.vector(data_ind$vmax), cvv_moy=as.vector(data_ind$cvv),ex_hours=as.vector(data_ind$nb_ex_cr)
)

gener_inits =function(){
  
  list("a"=matrix(data=rnorm(22,mean=0,sd=1),nrow=2,ncol=11),
       "k"=c(runif(1,0,1),runif(1,2,5)),"inv_lambda"=runif(2,0,1),
       
       "alpha"=c(runif(1,1,2),runif(1,2,5)),"beta"=c(runif(1,3,7),runif(1,1,2)),
       "mu"=c(runif(1,2,5),runif(1,0,1)),"tau"=runif(2,0,2)
       
       ,"phi"=c(rnorm(1,mean=pi, sd=0.1), runif(1,0,1))
       ,"rho"=runif(2,0,2)
       
  )
}


inits=list(gener_inits(),gener_inits(), gener_inits())
print(inits)
##############################################"
#library(Rmpi)
library(doParallel)
library(coda)
library(runjags)

cl <- makeCluster(3)

registerDoParallel(cl)
clusterCall(cl, function () Sys.info () [c ( "nodename", "machine" ) ] )

clusterExport(cl,c("mydata", "inits"))
clusterEvalQ(cl,library(rjags))
clusterEvalQ(cl,library(coda))
clusterEvalQ(cl,library(runjags))
clusterEvalQ(cl,library(doParallel))
clusterEvalQ(cl,library(foreach))
clusterEvalQ(cl,library(iterators))
clusterEvalQ(cl, load.module("vonmises", path="/home/lamonicad/lib/JAGS/modules-4"))

#https://github.com/yeagle/jags-vonmises

lesvariables=c("k", "inv_lambda","phi", "rho", "mu", "tau", "alpha", "beta", "a"
)

debut=Sys.time()
print(debut)

jags_res=foreach(i =1:3,.combine='mcmc.list',.multicombine=TRUE)%dopar%{
  as.mcmc(run.jags("modele_individu_v9",monitor=lesvariables,data=mydata,n.chains=1,inits=dump.format(inits[[i]]),
              burnin=200,sample=500,thin=100,tempdir=FALSE,
                   summarise=FALSE,adapt = 100,keep.jags.files=FALSE))

}



fin=Sys.time()
print(fin)

stopCluster(cl)

save.image(paste("image_inf_individual_",numpoiss_vect[num_ind],"_v10.Rdata", sep=""))


